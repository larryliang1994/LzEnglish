...
//        });
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_VOICE_CHANGE:
                    //剩余10s
                    showRemainedTime();
                    mDialogManager.updateVoiceLevel(mAudioManager.getVoiceLevel(9));
                    break;
    ...
    private void showRemainedTime() {
        ...
        if (remainTime < mRemainedTime) {
            ...
            mDialogManager.getCancelTextView().setText("还可以说" + remainTime + "秒");
        }
    ...
    private void changeState(int state) {
        if (mCurrentState != state) {
            ...
            switch (mCurrentState) {
                case STATE_NORMAL:
                    setText("按住录音 跟读");//长按录音
                    break;
                case STATE_RECORDING:
                    setBackground(ContextCompat.getDrawable(mContext, R.drawable.round_button_login));
